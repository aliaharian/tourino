...
import { codeSend, codeVerifyLogin, enqueueSnackbar, setAuthStep } from "../../../redux/user"
        ...
        switch (props.step) {
            ...
            case 'enterCode':
                if (code[0] !== '' && code[1] !== '' && code[2] !== '' && code[3] !== '') {
                    Dispatch(codeVerifyLogin(numberFormat.toEnglishDigitsOnlyNum(number), code[0] + code[1] + code[2] + code[3]))
                } else {
                    setError(true)
                    Dispatch(enqueueSnackbar(
                        {
