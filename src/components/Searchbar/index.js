...
const Searchbar = ({ scrollPos, showSearch, setShowSearch, transport, query, dest, origin }) => {
    ...
    useEffect(() => {
        ...
        if (
            ...
            if (startTmp < today || endTmp < today || startTmp > endTmp || Math.round((endTmp - startTmp) / (1000 * 60 * 60 * 24)) < 2) {
                Router.push('/', undefined, { shallow: true })
                Dispatch(enqueueSnackbar(
                    {
                        message: 'پارامتر های ارسالی صحیح نیست',
        ...
        if (query.startDate && query.endDate) {
            setStartDateStr(dateTime.dateTimeCustom(query.startDate).day + " " + dateTime.dateTimeCustom(query.startDate).month + " ")
            setEndDateStr(dateTime.dateTimeCustom(query.endDate).day + " " + dateTime.dateTimeCustom(query.endDate).month + " ")
        }
        if (query.rooms) {
            let tmp = query.rooms
            let roomsArray = []
    ...
    return (
        <div ref={anchorElGlobal}
            className={clsx(classes.searchbar, (scrollPos && !showSearch) && classes.searchbarSm)}>
            {!scrollPos || showSearch ?
                ...
                :
                <div className={clsx(classes.searchbarPlaceholder , !(query && startLocation && endLocation && startDateStr) && classes.emptyPlaceholder)} onClick={() => setShowSearch(true)}>
                    {(query && startLocation && endLocation && startDateStr) ?

                        <Typography>
                            {startLocation?.name}<span> | </span>{endLocation?.name}<span> | </span>{startDateStr} _ {endDateStr}<span> | </span>{parseInt(adults) + parseInt(infants)} نفر
                     </Typography>
                        :
                        <Typography>
                            جستجو کنید
